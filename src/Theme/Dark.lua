...
Palettes.dark = {
	...
	-- enemy bar's two chambers, iron is the frame round a bar, the swing hands
	-- are the timer's two fills, and bar is the one colour the cast bar and the
	-- experience rail share. What a colour means, threat and reaction and
	-- class, stays in Unit/Color.lua for the reason the item grades stay in
	-- UI/Theme.lua. The rested pool beyond the experience rail is the palette's
	-- accent and has no entry here.
	--
	-- Bar is shaped after it is painted, so a palette may write any brightness
	-- here and the name on the cast bar still clears its contrast floor.
	unit = {
